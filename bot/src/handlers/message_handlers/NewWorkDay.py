...
from src.services.redis.storageTables import storage_colected_new_work_day
...
async def cash_in_handler(message: types.Message, state: FSMContext):
    ...
    while is_valid_integer(cash_in):
        ...
        await state.update_data(cash_in=cash_in)
        await message.answer("please write total amount of spending")
        await storage_colected_new_work_day.cash_out.set()
        break
    else:
...
async def cash_out_handler(message: types.Message, state: FSMContext):
    ...
    while is_valid_integer(cash_out):
        logger.info(f"cash out {cash_out}", message.chat.id)
        await state.update_data(cash_out=cash_out)
...
cash_in: {cash_in}    
cash_out: {cash_out}
""", reply_markup=inline.CashBoxAddShift.CashBoxSendAndInsert()
        )
...
def register_message_handlers_new_work_day(dp: Dispatcher):
    dp.register_message_handler(cash_in_handler, state=storage_colected_new_work_day.cash_in)
    dp.register_message_handler(cash_out_handler, state=storage_colected_new_work_day.cash_out)
